...
import { changeCategory, addBrand, removeBrand, addColor, removeColor } from '../../services/filter/actions'
...
class WidgetFilter extends React.Component {
	componentDidMount() {
 	...
 	render() {
 		const { translate, lang, filter, changeCategory, addBrand, removeBrand, addColor, removeColor } = this.props;
		return (
			<>
				<div className="box box-with-categories" id="mfilter-box-32">
					<h3 className="box-heading">{translate('megafilter_pro')}</h3>
						...
						<ul>
													...
													<ul className="mfilter-tb" data-top-url="" data-top-path="0">
														{filter.category > 0 ? (
															<li className="mfilter-to-parent">
																<a href="#category" onClick={() => changeCategory(0)}>
																	{categories.map(category => {
																		if(category.id === filter.category) {
																			return (
																				<span key={category.id}>{category.title[lang.locale]}</span>
																			)
																		} else {
																			return false;
																		}
															...
															<>
																{categories.map(category => {
																	return (
																		<li className="mfilter-tb-as-tr" key={category.id}>
																			<div className="mfilter-tb-as-td">
																				<a href="#category" onClick={() => changeCategory(category.id)}>{category.title[lang.locale]}</a>
																			</div>
																		</li>
																	);
																})}
															...
															return (
																<div key={color.id}>
																	{filter.colors.indexOf(color.id) > -1 ? (
																		<li className="mfilter-option mfilter-image mfilter-visible mfilter-image-checked" onClick={() => removeColor(color.id)} key={color.id}>
																			<input type="checkbox" style={{ display: 'none '}} />
																			<label>
																				<img src={color.image} alt={color.id} />
																			</label>
																		</li>
																	) : (
																		<li className="mfilter-option mfilter-image mfilter-visible" onClick={() => addColor(color.id)} key={color.id}>
																			<input type="checkbox" style={{ display: 'none '}} />
																			<label>
																				<img src={color.image} alt={color.id} />
																			</label>
							...
							<li className="mfilter-filter-item mfilter-checkbox mfilter-manufacturers">
								<div className="mfilter-heading">
									<div className="mfilter-heading-content">
										<div className="mfilter-heading-text">
											<span>{translate('manufacturers')}</span>
										</div>
											<i className="mfilter-head-icon"></i>
									</div>
								</div>
															...
															return (
																<div key={brand.id}>
																	{filter.brands.indexOf(brand.id) > -1 ? (
																		<div className="mfilter-option mfilter-tb-as-tr mfilter-visible" onClick={() => removeBrand(brand.id)} key={brand.id}>
																			<div className='mfilter-tb-as-td mfilter-col-input mfilter-input-active'>
																				<input id={'mfilter-opts-attribs-32-manufacturers-' + brand.id} name="manufacturers" type="checkbox" readOnly={true} checked={true} />
																			</div>
																			<label className="mfilter-tb-as-td">{brand.title[lang.locale]}</label>
																		</div>
																	) : (
																		<div className="mfilter-option mfilter-tb-as-tr mfilter-visible" onClick={() => addBrand(brand.id)} key={brand.id}>
																			<div className='mfilter-tb-as-td mfilter-col-input'>
																				<input id={'mfilter-opts-attribs-32-manufacturers-' + brand.id} name="manufacturers" type="checkbox" readOnly={true} checked={false} />
																			</div>
																			<label className="mfilter-tb-as-td">{brand.title[lang.locale]}</label>
																		</div>
																	)}
...
export default connect(
    mapStateToProps, { changeCategory, addBrand, removeBrand, addColor, removeColor }
